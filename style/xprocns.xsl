...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="cx p xs db"
                version="2.0">
...
<xsl:template match="p:declare-step">
  <p class="element-syntax element-syntax-declare-step">
    <span class="decl">
...
<xsl:template match="*" mode="xprocelem">
  ...
  <span>
    ...
    <xsl:for-each select="@*[not(name()='name' or name()='port'
                             or namespace-uri()='http://xmlcalabash.com/ns/extensions')]">
      <xsl:call-template name="doAttr"/>
    </xsl:for-each>
	...
	<xsl:for-each select="@*[not(namespace-uri()='http://xmlcalabash.com/ns/extensions')]">
	...
	<xsl:choose>
	  <xsl:when test="not(@cx:type)">
	    <xsl:message>
              <xsl:text>Warning: no @cx:type on </xsl:text>
              <xsl:value-of select="@name"/>
              <xsl:text> on </xsl:text>
              <xsl:value-of select="ancestor::p:declare-step/@type"/>
            </xsl:message>
	    <xsl:value-of select="'string'"/>
	  </xsl:when>
	  <xsl:when test="contains(@cx:type,'|')">
	    <xsl:for-each select="tokenize(@cx:type,'\|')">
	  ...
	  <xsl:otherwise>
	    <xsl:value-of select="replace(@cx:type,'xsd:','')"/>
	  </xsl:otherwise>
	</xsl:choose>
      </xsl:variable>
